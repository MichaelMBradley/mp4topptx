...
class statusbar:
    def printstatus(self):
        ...
        remaining = 1 - percent
        print(f"\r{self.msg} | Completed {self.curr}/{self.length} |" + "█" * int(20 * percent) + "░" * int(20 * remaining) + f"| {percent:.0%} | Remaining: {formattime((self.length - self.curr) * self.rate, decimals=1)}" + " " * 5, end="", flush=True)
